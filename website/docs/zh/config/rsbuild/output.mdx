...
`wasm` 路径适用于 [`lib.wasm.mode: 'compile'`](/config/lib/wasm#wasmmode) 输出的 WebAssembly。bundleless Preserve 模式会保留每个 `.wasm` 文件的源码相对路径。
...
`output.filenameHash` 不会影响 bundleless [`lib.wasm.mode: 'preserve'`](/config/lib/wasm#wasmmode) 输出的 WebAssembly 文件。preserve 模式下的 `.wasm` 文件会使用源文件名。
...
`output.filename` 不会重命名 bundleless [`lib.wasm.mode: 'preserve'`](/config/lib/wasm#wasmmode) 输出的 WebAssembly 文件。preserve 模式下的 `.wasm` 文件会使用源文件名。
