...
- **类型：** `string`
- **默认值：** 项目的 `typescript` 依赖的 `package.json` 路径

指定用于生成类型声明文件的 TypeScript。配置值必须是指向这个 TypeScript 的 `package.json` 的绝对路径。

未设置此项时，Rslib 根据项目的 `typescript` 依赖的版本选择后端：TypeScript 5 和 6 使用 Compiler API，TypeScript 7 及以上版本使用 native executable。设置此项后，Rslib 改为根据这个 TypeScript 的版本选择后端。
...
由于 isolated 类型声明生成不会使用 TypeScript，因此该配置不能与 [dts.isolated](#dtsisolated) 同时使用。
...
如果没有显式设置该选项，Rslib 会在检测到 TypeScript 7+ 时自动开启。设置 [dts.typescriptPath](#dtstypescriptpath) 后，Rslib 会根据这个 TypeScript 的版本检测。
