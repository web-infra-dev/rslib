...
For more complex scenarios, you can directly use Rspack's [BannerPlugin](/config/lib/banner#customize-banner-content) to achieve this.
...
Tailwind CSS v4 cannot be used with CSS preprocessors like Sass, Less, or Stylus. When using Tailwind v4, make sure the `@import 'tailwindcss';` statement appears at the beginning of a plain `.css` file so that it can be processed correctly. See [Tailwind CSS - Compatibility](https://tailwindcss.com/docs/compatibility#sass-less-and-stylus).
...
If you need to enable CSS minification, you can set `output.minify.css` to `true` via the [output.minify](/config/rsbuild/output#outputminify) option. Rslib will enable Rspack's built-in [LightningCssMinimizerRspackPlugin](https://rspack.rs/plugins/rspack/lightning-css-minimizer-rspack-plugin) to minify CSS assets.

:::note

Note that the `output.minify` option you configure will completely override Rslib's default configuration. See the [output.minify](/config/rsbuild/output#outputminify) documentation for more details.
