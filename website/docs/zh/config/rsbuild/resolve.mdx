---
description: '配置 Rslib 中的 Rsbuild resolve 选项，包括别名、扩展名、包解析、条件导出与模块查找规则。'
---
...
::: note
在 bundleless 模式下，由于没有打包这个概念，所有模块都会被外部化，Rslib 会自动根据 `resolve.alias` 或 `tsconfig.json` 中 [compilerOptions.paths](https://www.typescriptlang.org/tsconfig/#paths) 配置的映射关系，对最终解析到 [outBase](/config/lib/out-base) 目录下的模块进行转换。
:::

## resolve.conditionNames <RsbuildDocBadge path="/config/resolve/condition-names" text="resolve.conditionNames" />

指定用于匹配包 [exports 字段](https://nodejs.org/api/packages.html#exports) 入口点的 condition names（条件名称）。
