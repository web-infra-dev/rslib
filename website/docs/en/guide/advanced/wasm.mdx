...
When using TypeScript, first add the [preset types](/guide/basic/typescript#preset-types) provided by `@rslib/core` to `tsconfig.json`:
...
This declaration extends the `*?inline` module declared in the preset types. Without the preset types, TypeScript ignores it. The `export {}` at the top marks the declaration file as a module, allowing TypeScript to recognize the relative module declaration below. You can then use a named import:
