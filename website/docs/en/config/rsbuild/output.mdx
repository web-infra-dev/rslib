...
`output.filenameHash` does not affect WebAssembly files emitted by [`lib.wasm.mode: 'preserve'`](/config/lib/wasm#wasmmode). Preserve mode uses the source file name for `.wasm` files.
...
`output.filename` does not rename WebAssembly files emitted by [`lib.wasm.mode: 'preserve'`](/config/lib/wasm#wasmmode). Preserve mode uses the source file name for `.wasm` files.
