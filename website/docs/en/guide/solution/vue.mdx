...
To develop Vue components, you need to set [target](/config/rsbuild/output#outputtarget) to `"web"` in `rslib.config.ts`. This is crucial because Rslib sets `target` to `"node"` by default, which is different from Rsbuild's default target value.

To compile Vue (.vue single-file components), you need to register the Rsbuild [Vue plugin](https://github.com/rspack-contrib/rsbuild-plugin-unplugin-vue). This plugin will automatically add the necessary configurations for Vue building.
