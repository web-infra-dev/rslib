...
推荐使用 [@rsbuild/plugin-tailwindcss](https://rsbuild.rs/zh/plugins/list/plugin-tailwindcss) 插件在 Rslib 中接入 [Tailwind CSS v4](https://tailwindcss.com/)。
...
如果你的项目已有 PostCSS 配置，也可以安装 `tailwindcss` 和 `@tailwindcss/postcss`，并通过 [postcss.config.mjs](https://npmjs.com/package/postcss-loader#config) 或 [tools.postcss](/config/rsbuild/tools#toolspostcss) 来注册 Tailwind CSS 的 PostCSS 插件。
