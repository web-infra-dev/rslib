...
Most `loadConfig` options are aligned with Rsbuild. The main differences in Rslib are that the default lookup list uses `rslib.config.*`, and the returned `content` is typed as `RslibConfig`.
