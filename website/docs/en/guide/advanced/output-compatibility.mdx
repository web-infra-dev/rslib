---
description: 'Control Rslib output compatibility with syntax targets, browsers, Node.js versions, and platform settings.'
---
...
By default, the syntax is set to `ESNext`, which only supports the latest version of mainstream browsers (Chrome / Firefox / Edge / macOS Safari / iOS Safari) or Node.js according to [output.target](/config/rsbuild/output#outputtarget). When `output.target` is set to `'node'`, Rslib will infer the syntax from the minimum Node.js version declared in `package.json#engines.node` if available.
