...
对于面向现代环境的库（例如浏览器应用或支持 ESM 的 Node.js 应用），仅发布 ESM 通常是最佳选择。当 ESM 模块图中不包含顶层 `await` 时，[Node.js 20.19+ 和 22.12+](https://nodejs.org/api/modules.html#loading-ecmascript-modules-using-require) 可以使用 `require()` 加载它；使用旧版 Node.js 或加载包含顶层 `await` 的 ESM 时，CommonJS 消费者仍需使用动态导入，例如 `const pureEsmLib = await import('pure-esm-lib')`。
...
社区正在向 ESM 迁移，但部分项目仍需支持旧版 Node.js，或无法通过 `require()` 加载 ESM 的工具。如果需要支持这些环境，可以发布一个双格式包。你可以阅读 Anthony Fu 的博客文章 [在一个包中发布 ESM 和 CJS](https://antfu.me/posts/publish-esm-and-cjs) 了解更多详情。
