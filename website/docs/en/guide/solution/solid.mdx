...
When an application project that uses this component library configures `@rsbuild/plugin-solid`, the `"solid"` condition name is automatically added to module resolution, so the application-side toolchain resolves the `.jsx` source and compiles it locally. Application projects without Solid JSX compilation configured will fall back to the pre-compiled `"import"` entry.
