---
description: 'Configure Rslib with lib options, shared Rsbuild settings, config files, and defineConfig.'
---
...
Some `lib` options can be configured outside the `lib` field as common defaults. Each `lib` item inherits these defaults, and the same option inside a `lib` item overrides the common value.

For example, set [`syntax`](/config/lib/syntax) once for all outputs:
...
The supported common `lib` options are listed in the [`lib` configuration overview](/config/lib).
