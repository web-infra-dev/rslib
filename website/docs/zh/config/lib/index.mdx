...
`lib` 是一个对象数组，用于描述 Rslib 生成的库产物。每个 `lib` 项对应一份产物，可以包含 lib 配置和 [Rsbuild 配置](/config/rsbuild)，详情请参考[配置 Rslib](/guide/basic/configure-rslib#配置结构)。

- **类型:** `LibConfig[]`（参考 [`RslibConfig`](/api/javascript-api/types#rslibconfig)）
- **默认值:** `undefined`
- **必选:** `true`
