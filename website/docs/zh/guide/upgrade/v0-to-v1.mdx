...
在 [bundleless 模式](/config/lib/bundle)下，Rslib v0.x 会生成加载和实例化 Wasm 模块所需的 JavaScript 代码，Rslib v1 则默认使用 `preserve` 模式，不再生成这些代码。如果不希望保留 `.wasm` import，可以将 [`wasm.mode`](/config/lib/wasm#wasmmode) 设置为 `'compile'`：
