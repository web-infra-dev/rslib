---
description: '配置 Rslib 中的 Rsbuild output 选项，包括资源前缀、输出目录、externals、文件名与 CSS 输出。'
---
...
- 当 `format` 为 `mf`、`umd` 或 `iife` 时，默认值为 `"/"`。

当 `output.assetPrefix` 设置为 `"auto"` 时，Rslib 会自动将 [importMode](https://rspack.rs/zh/config/module-generator#assetimportmode) 设置为 `"preserve"`，以保留 JavaScript 文件中引用静态资源的 `import` 或 `require` 语句。同时，CSS 文件中的静态资源会保留相对引用路径，详情可以查看 [静态资源](/guide/advanced/static-assets)。

当 `output.assetPrefix` 设置为具体的路径时，JavaScript 文件中引用静态资源的 `import` 或 `require` 语句会被替换为包含该前缀的 URL。同时，CSS 文件中的静态资源会被直接替换为带有该前缀的路径。

## output.autoExternal <RsbuildDocBadge path="/config/output/auto-external" text="output.autoExternal" />

- **命令行：** `--auto-external` / `--no-auto-external`

是否自动对不同依赖类型的依赖进行外部化处理，不将其打包。该配置仅在 bundle 模式下生效，bundleless 模式下不会生效。

在 Rslib 中，该选项的默认值由 [format](/config/lib/format) 决定：

- 当 `format` 为 `cjs` 或 `esm` 时，默认值为 `true`。
- 当 `format` 为 `umd`、`mf` 或 `iife` 时，默认值为 `false`。

设置为 `true` 时，`package.json` 中 `dependencies`、`optionalDependencies` 和 `peerDependencies` 字段下的依赖将被自动外部化。
...
- **命令行：** `--dist-path <dir>`（例如 `--dist-path output`）
...
其中，`wasm` 配置仅在 [lib.wasm.mode](/config/lib/wasm#wasmmode) 为 `compile` 时生效，用于指定 `.wasm` 文件的输出目录。在 `preserve` 模式下，`.wasm` 文件会按源码相对路径输出。
...
在 bundle 模式下，Rslib 会根据 [output.autoExternal](#outputautoexternal) 自动将 `package.json` 中的依赖进行外部化处理，查看 [output.autoExternal](#outputautoexternal) 了解更多信息。

此外，Rslib 会根据产物格式设置 Rspack 的 [`externalsType`](https://rspack.rs/zh/config/externals#externalstype) 默认值：

- `esm`：[modern-module](https://rspack.rs/zh/config/externals#externalstypemodern-module)，静态 `import` 的 external 会被输出为 ESM import，动态 `import()` 会保持动态导入；CommonJS `require()` 的 external 在 Node-like [`output.target`](#outputtarget) 下会通过 `createRequire` 加载，在其他 target 下会保留裸 `require()` 调用。

:::warning
对于通过 `createRequire()` 加载的 external 模块，如果对应产物需要再次打包，消费方的打包器必须能够静态分析这种调用模式。Rspack 项目可开启 [`module.parser.javascript.createRequire`](https://rspack.rs/zh/config/module-parser#javascriptcreaterequire) 来支持该能力。
:::

- `cjs`：[commonjs-import](https://rspack.rs/zh/config/externals#externalstypecommonjs-import)。
- `umd`：[umd](https://rspack.rs/zh/config/externals#externalstype)。
- `mf` / `iife`：[global](https://rspack.rs/zh/config/externals#externalstypeglobal)。
...
对于 `esm`、`cjs`、`umd` 和 `iife` 格式，Rslib 默认将 `output.filenameHash` 设置为 `false`。对于 `mf` 格式，Rslib 保留 Rsbuild 的默认值，在生产模式下为产物的文件名添加 hash 值。

::: info 文件名中的 hash 值

可以将 `output.filenameHash` 设置为 `true`，在产物的文件名中添加 hash 值，或设置为 `false` 来禁用。
...
- 当 format 为 `iife` 时，Rslib 默认也仅执行死代码消除和未使用代码消除，并将 `minimizerOptions.module` 设置为 `true`。
...
- **命令行：** `--target <target>`（例如 `--target web`）

用于设置 Rsbuild 的构建产物类型。

对于 `esm`、`cjs`、`umd` 和 `iife` 格式，Rslib 默认将 `output.target` 设置为 `node`；对于 `mf` 格式，默认值为 `web`。
