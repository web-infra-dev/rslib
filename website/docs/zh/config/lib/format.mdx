---
description: '配置 lib.format，选择 Rslib 的输出格式，包括 ESM、CJS、UMD、Module Federation 与 IIFE。'
---
...
- `iife`：开启了 [output.iife](https://rspack.rs/zh/config/output#outputiife) 的 [module](https://rspack.rs/zh/config/output#type-module)

更多详情请参考 [输出格式](/guide/basic/output-format) 和 [模块联邦](/guide/advanced/module-federation)。

不同产物格式会影响 Rslib 的部分默认行为，详情请参阅 [默认行为](/config/default-behavior)。
