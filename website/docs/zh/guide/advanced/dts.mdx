---
description: '在 Rslib 中生成和打包 TypeScript 声明文件，涵盖 isolatedDeclarations 与相关最佳实践。'
---
...
Rslib 支持使用 [TypeScript Compiler API](https://github.com/microsoft/TypeScript/wiki/Using-the-Compiler-API)、[tsgo](https://github.com/microsoft/typescript-go) 和 [`isolatedDeclarations`](https://www.typescriptlang.org/tsconfig/#isolatedDeclarations) 生成类型声明文件，并且支持使用 [API Extractor](https://api-extractor.com/) 将生成的类型声明文件打包。
...
### 使用 isolatedDeclarations 生成类型声明文件

你可以开启 [dts.isolated](/config/lib/dts#dtsisolated)，使用 Rspack 内置的 SWC fast_dts 能力生成类型声明文件：
...
关于适用场景、输出范围和使用条件，请参考 [dts.isolated](/config/lib/dts#dtsisolated) 配置文档。
