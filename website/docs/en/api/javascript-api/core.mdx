...
Most `loadConfig` options follow the same usage as Rsbuild. In Rslib, the default lookup list uses `rslib.config.*`, and the returned `content` is typed as [RslibConfig](/api/javascript-api/types#rslibconfig); see [loadConfig -- Rsbuild](https://rsbuild.rs/api/javascript-api/core#loadconfig) for more usage details.
