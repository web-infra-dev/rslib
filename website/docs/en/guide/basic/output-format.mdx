...
Shipping only ESM is the best choice for libraries intended for modern environments, such as browser applications or Node.js applications that support ESM. [Node.js 20.19+ and 22.12+](https://nodejs.org/api/modules.html#loading-ecmascript-modules-using-require) can load ESM using `require()` when its module graph does not contain top-level `await`. CommonJS consumers on older Node.js versions, or those loading ESM with top-level `await`, should use dynamic import, such as `const pureEsmLib = await import('pure-esm-lib')`.
...
The community is migrating to ESM, but some projects still need to support older Node.js versions or tools that cannot load ESM using `require()`. If you need to support those environments, you can publish a dual package. Read Anthony Fu's blog post [Publish ESM and CJS packages](https://antfu.me/posts/publish-esm-and-cjs) for more details.
