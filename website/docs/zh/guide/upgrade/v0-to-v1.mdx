...
Rslib v1 不再将 Rsbuild 默认的环境变量预设内联到 ESM 和 CJS 库产物中。对于 ESM 产物，`import.meta.env` 和 `process.env.*` 引用会被保留，交由下游打包器替换或目标运行时解析。对于 CJS 产物，`process.env.*` 引用会被保留，而 `import.meta.env.*` 引用会被替换为 `undefined`，因为 CommonJS 中不存在 `import.meta`。UMD 等其他格式的行为保持不变。
