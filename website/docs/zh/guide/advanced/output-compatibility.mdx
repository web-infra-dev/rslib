---
description: '通过语法目标、浏览器、Node.js 版本与平台配置，控制 Rslib 产物的兼容性范围与目标环境。'
---
...
默认情况下，syntax 被设置为 `ESNext`，这代表将仅支持主流浏览器（Chrome / Firefox / Edge / macOS Safari / iOS Safari）或 Node.js 的最新版本，具体取决于 [output.target](/config/rsbuild/output#outputtarget)。当 `output.target` 设置为 `'node'` 时，如果 `package.json#engines.node` 声明了最低 Node.js 版本，Rslib 会根据该版本推断语法目标。
