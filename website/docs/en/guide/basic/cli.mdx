---
description: 'Use the Rslib CLI for build, inspect, watch mode, config selection, and common command-line options.'
---
...
The `rslib --watch` command builds in watch mode. If you need to use different configuration in watch mode, check whether `--watch` is present in `process.argv`:
