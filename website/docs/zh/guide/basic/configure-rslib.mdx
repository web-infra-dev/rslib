---
description: '配置 Rslib，了解 lib 选项、共享的 Rsbuild 配置、配置文件与 defineConfig 的使用方式。'
---
...
部分 `lib` 配置可以配置在 `lib` 字段外部，作为公共默认值。每个 `lib` 项都会继承这些默认值，`lib` 项内的同名配置会覆盖公共配置。

例如，为所有产物统一设置 [`syntax`](/config/lib/syntax)：
...
支持的公共 `lib` 配置项见 [`lib` 配置总览](/config/lib)。
