...
- `true`: when [format](/config/lib/format) is `cjs`, the `import.meta.dirname` in source code will be replaced with the `__dirname` of the current module.

- `false`: the `import.meta.dirname` will be left as is, which will cause a runtime error when running the output in Node.js < 20.11.0 or < 21.2.0.
...
- `true`: when [format](/config/lib/format) is `cjs`, the `import.meta.filename` in source code will be replaced with the `__filename` of the current module.

- `false`: the `import.meta.filename` will be left as is, which will cause a runtime error when running the output in Node.js < 20.11.0 or < 21.2.0.
