...
- [Rsbuild configurations](/config/rsbuild) control the underlying build behavior.

The `lib` field is an array of objects. Each object corresponds to one output and can contain both types of configuration above. Configurations inside a `lib` item apply only to that output, while configurations outside the `lib` field are shared across all outputs.
