...
将指定的文件或目录拷贝到构建输出目录中，基于 [rspack.CopyRspackPlugin](https://rspack.dev/plugins/rspack/copy-rspack-plugin) 实现。
...
是否生成 manifest 文件，该文件包含所有构建产物的信息、以及[入口模块](https://rsbuild.dev/config/source/entry)与构建产物间的映射关系。
