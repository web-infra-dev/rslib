---
description: 'Configure Rslib with lib configurations, Rsbuild configurations, config files, and defineConfig.'
---
...
Rslib has two types of configuration:

- [Lib configurations](/config/lib): Describe library outputs, such as their module formats, bundle modes, and syntax targets.
- [Rsbuild configurations](/config/rsbuild): Control compilation and build behavior, such as source transforms and output paths.

Rslib configurations can be specified inside the `lib` option or at the top level. The `lib` option is an array of objects, with each object configuring one output. Configurations in a `lib` item customize that output, while top-level configurations apply to all outputs.

### Lib configurations

Lib configurations are usually specified in a `lib` item to describe an individual output. Some can also be specified at the top level and shared by all outputs. The same configuration in a `lib` item takes precedence over the shared configuration.

For example, all outputs use [`syntax`](/config/lib/syntax) `es2021` by default, while the CJS output uses `es2020`:
...
Not all lib configurations can be shared. For example, `format` and `dts` describe an individual output and can only be specified in a `lib` item. See [Lib configurations](/config/lib) for the complete list.

### Rsbuild configurations

Rsbuild configurations can be specified at the top level to apply to all outputs, or in a `lib` item to customize that output. Rsbuild configurations in a `lib` item are merged with the top-level configurations and take precedence when they conflict.

For example, the [output.target](/config/rsbuild/output#outputtarget) defaults to `node` for all outputs, while the ESM output uses `web`:
