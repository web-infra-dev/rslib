...
Rslib 支持通过 `@rsbuild/plugin-babel` 与 `babel-plugin-react-compiler` 启用 [React Compiler](https://zh-hans.react.dev/learn/react-compiler)。

使用 `create-rslib` 时，React Compiler 是一个可选工具，只有在你显式选择可选工具中的 `React Compiler`，或在非交互模式传入 `--tools react-compiler` 时才会启用。

React Compiler 需要 React 19 或更高版本。在启用了 React Compiler 的脚手架项目中，`peerDependencies` 应声明为 `"react": ">=19.0.0"`。
