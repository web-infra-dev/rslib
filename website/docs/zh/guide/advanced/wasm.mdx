...
`?inline` 导入不受 [lib.wasm.mode](/config/lib/wasm#wasmmode) 影响。将 [lib.wasm](/config/lib/wasm) 设置为 `false` 可阻止 Rslib 处理带有 `?inline` query 的 `.wasm` 模块，`?inline` 将会被保留至产物中。
...
使用 TypeScript 时，请先在 `tsconfig.json` 中添加 `@rslib/core` 提供的 [预设类型](/guide/basic/typescript#预设类型)：
...
该声明扩展的是预设类型中声明的 `*?inline` 模块，未引入预设类型时 TypeScript 会忽略它。文件开头的 `export {}` 用于将声明文件标记为一个模块，让 TypeScript 能够识别下面的相对路径模块声明。声明后即可使用命名导入：
