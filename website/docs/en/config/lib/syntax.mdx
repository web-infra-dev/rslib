...
- **Default:** `'esnext'`, or inferred from `package.json#engines.node` when [output.target](/config/rsbuild/output#outputtarget) is `'node'`
...
When [output.target](/config/rsbuild/output#outputtarget) is set to `'node'` and `syntax` is not configured, Rslib will infer the syntax from the minimum Node.js version declared in `package.json#engines.node`.
