...
Rslib 的 `loadConfig` 与 Rsbuild 的用法基本一致，区别在于默认查找 `rslib.config.*`，且返回结果中的 `content` 类型为 [RslibConfig](/api/javascript-api/types#rslibconfig)。

以下选项的详细用法可参考 Rsbuild 文档：
