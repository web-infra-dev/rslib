...
- **类型：** `string`
- **默认值：** 从项目根目录解析到的 `typescript/package.json`

指定用于生成类型声明文件的 TypeScript 安装对应的 `package.json` 绝对路径。TypeScript 5 和 6 会使用 TypeScript Compiler API，TypeScript 7 及以上版本会使用 native executable。

当项目按照 TypeScript 官方推荐的方式通过 [npm alias 并存安装 TypeScript 6 和 7](https://devblogs.microsoft.com/typescript/announcing-typescript-7-0/#running-side-by-side-with-typescript-60) 时，可以使用此配置：
...
通过以下配置，可以显式选择使用 alias 安装的 TypeScript 7 生成类型声明文件：
...
该路径必须是 TypeScript `package.json` 的绝对路径。由于 isolated 类型声明生成不会使用 TypeScript，因此该配置不能与 [dts.isolated](#dtsisolated) 同时使用。
...
如果没有显式设置该选项，Rslib 会在检测到 TypeScript 7+ 时自动开启。设置 [dts.typescriptPath](#dtstypescriptpath) 后，Rslib 会根据该配置指定的 TypeScript 安装检测版本。
