...
<Prompt
  title="Upgrade from 0.x to v1"
  description="Copy this prompt and send it to your Coding Agent."
  prompt={`Upgrade this project from Rslib 0.x to 1.0. Read and follow the migration guide:
...
Rslib v1 changes how these variables are handled when [format](/config/lib/format) is `'esm'` or `'cjs'`:
...
For complete details on how Rslib v1 handles environment variables, see [Environment variables](/guide/advanced/env-vars).

If your project relies on Rslib v0.x replacing these variables at build time, explicitly define the variables it uses with [source.define](/config/rsbuild/source#sourcedefine) to restore the previous behavior. To use `import.meta.env.*` in CJS output, you must also explicitly define the corresponding variables:
