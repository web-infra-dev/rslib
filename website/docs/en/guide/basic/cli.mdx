...
Options:
  ...
  --entry <entry>        set entry file or pattern (repeatable) (e.g. --entry="src/*" or --entry="main=src/main.ts")
  --dist-path <dir>      set root output directory
  --bundle               enable bundle mode (use --no-bundle to disable)
  --format <format>      specify the output format (esm | cjs | umd | mf | iife)
  --syntax <syntax>      set build syntax target (e.g. --syntax=es2018 or --syntax=["node 14", "Chrome 103"])
  --target <target>      set runtime target (web | node)
  --dts                  emit declaration files (use --no-dts to disable)
  --external <pkg>       add package to externals (repeatable)
  --minify               minify output (use --no-minify to disable)
  --clean                clean dist directory before build (use --no-clean to disable)
