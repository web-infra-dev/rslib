...
In bundle mode, when format is esm or cjs, Rslib enables `output.autoExternal` by default, automatically externalizing dependencies declared in `dependencies`, `optionalDependencies`, and `peerDependencies`.
...
### Use function to configure external rules

For complex scenarios where you need to decide whether to externalize a module based on the request issuer or context, configure Rspack's `externals` via [tools.rspack](/config/rsbuild/tools#toolsrspack):
