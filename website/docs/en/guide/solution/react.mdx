...
Rslib supports enabling [React Compiler](https://react.dev/learn/react-compiler) through `@rsbuild/plugin-babel` and `babel-plugin-react-compiler`.

When using `create-rslib`, React Compiler is optional and is only enabled when you explicitly select `React Compiler` in optional tools, or pass `--tools react-compiler` in non-interactive mode.

React Compiler requires React 19 or higher. In scaffolded projects that enable React Compiler, the `peerDependencies` should be specified as `"react": ">=19.0.0"`.
