...
| `--env-dir <dir>`       | 指定目录来加载 `.env` 文件，详见 [Rsbuild - Env 目录](https://rsbuild.dev/zh/guide/advanced/env-vars#env-目录)             |
| `--env-mode <mode>`     | 指定 env 模式来加载 `.env.[mode]` 文件，详见 [Rsbuild - Env 模式](https://rsbuild.dev/zh/guide/advanced/env-vars#env-模式) |
| `-h, --help`            | 显示命令帮助                                                                                                               |
