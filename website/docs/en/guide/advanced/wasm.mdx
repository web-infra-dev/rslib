...
- When using `?inline`, [lib.wasm.mode](/config/lib/wasm#wasmmode) does not affect inlining. When [lib.wasm](/config/lib/wasm) is set to `false`, Rslib does not process the module and preserves the import with `?inline` in the output.
...
When using TypeScript, add the [preset types](/guide/basic/typescript#preset-types) provided by `@rslib/core` to `compilerOptions.types` in `tsconfig.json`. If this option already lists other type packages, append it to the existing array:
