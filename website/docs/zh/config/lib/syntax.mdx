...
- **默认值：**
  - `'esnext'`
  - 当 [output.target](/config/rsbuild/output#outputtarget) 为 `'node'` 时，会根据 `package.json#engines.node` 推断
...
当 [output.target](/config/rsbuild/output#outputtarget) 设置为 `'node'` 且未配置 `syntax` 时，Rslib 会读取 `package.json` 中 `engines.node` 声明的最低 Node.js 版本，并将其作为语法目标。
