...
对于 ESM 和 CJS 的 bundle 产物，Rslib 默认启用 `output.autoExternal`，会自动 external `dependencies`、`optionalDependencies` 和 `peerDependencies` 中声明的依赖。
...
如果需要根据请求来源、上下文等信息决定是否 external，可以通过 [tools.rspack](/config/rsbuild/tools#toolsrspack) 配置 Rspack 的 `externals`：
...
大多数场景优先使用 `output.autoExternal` 和 `output.externals`。只有在需要更复杂的 Rspack external 能力时，才建议使用 `tools.rspack.externals`。更多用法可参考 Rspack 的 [Externals](https://rspack.rs/zh/config/externals) 文档。
