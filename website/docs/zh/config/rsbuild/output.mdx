...
`output.filenameHash` 不会影响由 [`lib.wasm.mode: 'preserve'`](/config/lib/wasm#wasmmode) 输出的 WebAssembly 文件。preserve 模式下的 `.wasm` 文件会使用源文件名。
...
`output.filename` 不会重命名由 [`lib.wasm.mode: 'preserve'`](/config/lib/wasm#wasmmode) 输出的 WebAssembly 文件。preserve 模式下的 `.wasm` 文件会使用源文件名。
