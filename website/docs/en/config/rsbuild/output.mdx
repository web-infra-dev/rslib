...
The `wasm` path applies to WebAssembly emitted in [`lib.wasm.mode: 'compile'`](/config/lib/wasm#wasmmode). Bundleless Preserve mode keeps each `.wasm` file at its source-relative path.
...
`output.filenameHash` does not affect WebAssembly files emitted in bundleless [`lib.wasm.mode: 'preserve'`](/config/lib/wasm#wasmmode). Preserve mode uses the source file name for `.wasm` files.
...
`output.filename` does not rename WebAssembly files emitted in bundleless [`lib.wasm.mode: 'preserve'`](/config/lib/wasm#wasmmode). Preserve mode uses the source file name for `.wasm` files.
