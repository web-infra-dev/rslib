...
Rslib v1 不再将 Rsbuild 默认的环境变量预设内联到 ESM 和 CJS 库产物中。对于依赖 `import.meta.env` 或 `process.env.BASE_URL` / `process.env.ASSET_PREFIX` 在构建库时被替换为固定值的使用场景，这是一项破坏性变更。

对于 ESM 产物，这些预设变量的引用会被保留，交由下游打包器替换或目标运行时解析。对于 CJS 产物，`process.env.*` 引用会被保留，而 `import.meta.env.*` 引用会被替换为 `undefined`，因为 CommonJS 中不存在 `import.meta`。UMD 等其他格式的行为保持不变。

如果需要在构建库时固定某个值，可以通过 [source.define](/config/rsbuild/source#sourcedefine) 显式定义：
...
更多详情请参考 [Web Workers](/guide/advanced/web-workers)。
...
在 [bundleless 模式](/config/lib/bundle) 下，Rslib v0.x 会生成加载和实例化 Wasm 模块所需的 JavaScript 代码，Rslib v1 则默认使用 `preserve` 模式，在 JavaScript 中保留 `.wasm` import。如需改用 `compile` 模式，可以配置 [`wasm.mode`](/config/lib/wasm#wasmmode)：
...
[bundle 模式](/config/lib/bundle) 下的 Wasm 处理行为保持不变。
...
| v0.x `--template` 参数 | v1 `--template` 参数 |
| ---------------------- | -------------------- |
| `node-esm`             | `node`               |
| `node-esm-js`          | `node-js`            |
| `node-esm-ts`          | `node-ts`            |
