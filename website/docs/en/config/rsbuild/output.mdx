...
The `wasm` option only takes effect when [lib.wasm.mode](/config/lib/wasm#wasmmode) is `compile`, and specifies the output directory for `.wasm` files. In `preserve` mode, `.wasm` files are emitted at their source-relative paths.
