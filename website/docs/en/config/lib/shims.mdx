---
description: 'Configure lib.shims to enable the CommonJS and ESM shims required by specific library output targets.'
overviewHeaders: [2, 3]
---
...
- `true`: when [format](/config/lib/format) is `cjs`, the `import.meta.dirname` in source code will be replaced with the `__dirname`.
...
- `true`: when [format](/config/lib/format) is `cjs`, the `import.meta.filename` in source code will be replaced with the `__filename`.
