...
在开发组件库或者工具库的过程中，开发者不仅需要关注项目逻辑的实现，还要考虑项目的构建、调试、文档、测试等等和代码逻辑无关的工作。社区中现有的许多工具与方案，它们通常都能较好地满足其中个别需求，但对于不熟悉这些工具与方案的开发者而言，往往需要繁琐的配置工作或多种工具配合使用来满足这些需求。

Rslib 基于 Rspack 和 Rsbuild 实现，针对库开发场景的多样化需求，提供了一套完整的解决方案，可以有效解决库开发中的工具生态不完善、模块规范兼容成本高、产物优化不足等问题。Rslib 充分发挥了 Rspack 带来的构建性能优势，利用 webpack / Rspack 生态的优势很好地支持了模块联邦等功能，并针对 webpack 对 library ESM 产物的支持不足等问题进行了优化，能够产出高质量的支持 tree-shaking 的 ESM 产物。此外，Rslib 可以通过统一的配置，解决应用项目与库项目的构建配置难以复用与共享的问题，进而降低开发者的配置成本，提升开发效率与体验。
