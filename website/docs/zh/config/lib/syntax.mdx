...
- **默认值：** `'esnext'`，当 [output.target](/config/rsbuild/output#outputtarget) 为 `'node'` 时，也会根据 `package.json#engines.node` 推断
...
当 [output.target](/config/rsbuild/output#outputtarget) 设置为 `'node'` 且未配置 `syntax` 时，Rslib 会根据 `package.json#engines.node` 中声明的最低 Node.js 版本推断语法目标。
