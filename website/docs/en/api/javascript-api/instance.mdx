...
`rslib.context` is a read-only object that provides some context information, which can be accessed through the `context` property of the Rslib instance:
...
The root path of the current build, corresponding to the `cwd` option of the [createRslib](/api/javascript-api/core#createrslib) method.
...
```ts
type Action = 'build' | 'mf-dev' | 'inspect';
```

`context.action` is set when running CLI commands or calling Rslib instance methods:

- `build`: set when running [rslib build](/guide/basic/cli#rslib-build) or [rslib.build()](/api/javascript-api/instance#rslibbuild).
- `mf-dev`: set when running [rslib mf-dev](/guide/basic/cli#rslib-mf-dev) or [rslib.startMFDevServer()](/api/javascript-api/instance#rslibstartmfdevserver).
- `inspect`: set when running [rslib inspect](/guide/basic/cli#rslib-inspect) or [rslib.inspectConfig()](/api/javascript-api/instance#rslibinspectconfig).

- **Example:**

```ts
if (rslib.context.action === 'build') {
  // do something
}
```
...
Normalized Rslib configuration object in Rslib instance.

- **Type:**

```ts
type Config = RslibConfig;
```

### context.rsbuildConfig

Normalized Rsbuild configuration object in Rslib instance.

- **Type:**

```ts
type Config = RsbuildConfig;
```
