---
description: '配置 Rslib 中的 Rsbuild source 选项，包括入口、全局常量、装饰器、别名与源码转换配置。'
---
...
- **命令行：** `--entry <path>`（可重复，例如 `--entry index=src/index.ts` 或 `--entry src/index.ts`）
...
:::info
参考 [lib.bundle](/config/lib/bundle#设置入口) 进一步了解如何为 bundle 和 bundleless 项目设置入口。
:::
