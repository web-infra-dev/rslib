...
此外，Rslib 会根据产物格式设置 Rspack 的 `externalsType` 默认值：

- `esm`：[modern-module](https://rspack.rs/zh/config/externals#externalstypemodern-module)，静态 `import` 的 external 会被输出为 ESM import，动态 `import()` 会保持动态导入，而 CommonJS `require()` 的 external 会通过 `createRequire` 加载。
