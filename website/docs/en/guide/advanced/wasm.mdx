---
description: 'Import WebAssembly modules in Rslib libraries, configure compile or preserve output, or inline binaries into JavaScript with the ?inline query.'
---
...
`?inline` imports are not affected by [lib.wasm.mode](/config/lib/wasm#wasmmode). Setting [lib.wasm](/config/lib/wasm) to `false` prevents Rslib from processing `.wasm` modules with the `?inline` query, preserving `?inline` in the output.
...
When using TypeScript, create a declaration file next to the `.wasm` file and declare the named exports for the exact `?inline` import path:
...
Use the named exports declared by the WebAssembly binary. For example, the declaration above provides the `add` export.

You should keep imports with resource queries as an internal implementation detail of your package. Do not re-export the WASM module directly, as this keeps the `?inline` specifier in the generated declaration file.
