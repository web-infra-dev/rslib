...
describe('output config', () => {
  describe('chunkFileName', () => {
    test('should prefix index for multi-compiler builds', async () => {
      ...
      const { files, rspackConfig } = await buildAndGetResults({ fixturePath });
      ...
      expect(esm0BaseNames.some((n) => /^0~.+\.js$/.test(n))).toBeTruthy();
      expect(esm1BaseNames.some((n) => /^1~.+\.js$/.test(n))).toBeTruthy();
    });
