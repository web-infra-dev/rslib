---
description: '在 Rslib 中通过 SVGR 导入 SVG 文件，并在 React 组件与静态资源处理之间做选择。'
---
...
在 bundle 模式下，Rslib 支持 [@rsbuild/plugin-svgr](https://rsbuild.rs/zh/plugins/list/plugin-svgr) 下的所有用法，唯一的区别是当以 URL 形式使用 SVG 文件时，Rslib 会按照 [静态资源](/guide/advanced/static-assets) 在产物中对应保留静态资源的 `import` 语句。
