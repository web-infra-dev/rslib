...
## Using environment variables in source code

Rslib handles environment variable references differently for each output format:
...
To replace an environment variable with a fixed value when building the library, define it explicitly with [source.define](/config/rsbuild/source#sourcedefine):
