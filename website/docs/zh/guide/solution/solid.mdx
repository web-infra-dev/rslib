...
- **编译产物**：默认 ESM 入口，给普通打包器消费。
- **JSX 源码**：保留 JSX，并通过 `"solid"` condition name 暴露给 Solid-aware 的下游打包器。

`package.json` 中的 `"solid"` condition name 指向 JSX 源码。下游项目配置 `@rsbuild/plugin-solid` 后，可以解析到这份 JSX 源码，并根据自己的目标重新编译（DOM vs SSR）。
