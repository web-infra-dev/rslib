---
description: '配置 lib.shims，为特定库产物目标启用 CommonJS 与 ESM 所需的 shims 及兼容处理。'
overviewHeaders: [2, 3]
---
...
- `true`：当 [format](/config/lib/format) 为 `cjs` 时，源代码中的 `import.meta.dirname` 将被替换为 `__dirname`。
...
- `true`：当 [format](/config/lib/format) 为 `cjs` 时，源代码中的 `import.meta.filename` 将被替换为 `__filename`。
