...
By default, Rslib uses the new JSX transform, which is `runtime: 'automatic'`. The automatic runtime itself requires React `16.14.0` or higher, but the Rslib React templates use React `18.0.0` or higher as the peer dependency baseline, so `peerDependencies` should be specified as `"react": ">=18.0.0"` and `"react-dom": ">=18.0.0"`.
