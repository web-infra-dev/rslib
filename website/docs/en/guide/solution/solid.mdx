...
Solid component libraries conventionally provide two outputs:

- **Compiled output**: the default ESM entry for regular bundlers.
- **Source output**: preserves JSX and is exposed through the `"solid"` condition name for Solid-aware downstream bundlers.

The `"solid"` condition name in `package.json` points to the source output. After a downstream project configures `@rsbuild/plugin-solid`, it can resolve this JSX source and recompile it for its own target (DOM vs SSR).
