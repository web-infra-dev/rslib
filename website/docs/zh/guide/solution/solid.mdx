---
description: '使用 Rslib 构建 Solid 组件库，涵盖编译输出与源码 JSX 输出、"solid" condition name 与类型声明生成。'
---
...
- **保留 JSX 的源码产物**：不做 Solid 编译，只保留原始 JSX 语法，通过 `package.json` 的 `"solid"` condition name（条件名称）暴露出来。

能够编译 Solid JSX 的下游项目（如安装了 `@rsbuild/plugin-solid` 的 Rsbuild 项目），会命中 `"solid"` condition name、拿到 JSX 源码，然后根据自己的运行环境重新编译——浏览器端应用编译为客户端 DOM 渲染代码（`generate: 'dom'`），SSR 框架编译为服务端字符串渲染代码（`generate: 'ssr'`）。没有 Solid 插件的项目则回退到预编译的 `"import"` 入口。
...
当下游应用使用 `@rsbuild/plugin-solid` 时，`"solid"` condition name 会自动加入模块解析，打包器会解析到 `.jsx` 源码并在本地编译。未配置 Solid 编译能力的应用会回退到预编译的 `"import"` 入口。
