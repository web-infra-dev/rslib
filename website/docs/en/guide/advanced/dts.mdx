---
description: 'Generate and bundle TypeScript declaration files in Rslib, including isolatedDeclarations and related best practices.'
---
...
Rslib supports generating declaration files using the [TypeScript Compiler API](https://github.com/microsoft/TypeScript/wiki/Using-the-Compiler-API), [tsgo](https://github.com/microsoft/typescript-go), and [`isolatedDeclarations`](https://www.typescriptlang.org/tsconfig/#isolatedDeclarations), and also supports bundling declaration files with [API Extractor](https://api-extractor.com/).

| Type       | Supported Method                        | Description    |
| ---------- | --------------------------------------- | -------------- |
| bundleless | TypeScript Compiler API                 | Default method |
| bundleless | tsgo                                    |                |
| bundleless | isolatedDeclarations                    |                |
| bundle     | TypeScript Compiler API + API Extractor | Default method |
| bundle     | tsgo + API Extractor                    |                |
| bundle     | isolatedDeclarations + API Extractor    |                |
...
Rslib recommends enabling `dts.tsgo` to speed up declaration file generation.
...
### Generate declaration files with isolatedDeclarations

You can enable [dts.isolated](/config/lib/dts#dtsisolated) to generate declaration files with Rspack's built-in SWC fast_dts capability:
...
For use cases, output scope, and usage constraints, refer to the [dts.isolated](/config/lib/dts#dtsisolated) configuration documentation.
