---
description: '在 Rslib 库中导入 WebAssembly 模块，配置 compile 或 preserve 输出模式，或通过 ?inline query 将二进制内联到 JavaScript 中。'
---
...
根据配置文件中的 [bundle](/config/lib/bundle) 配置，在 `dist` 目录下输出如下产物：
...
- 使用 `?inline` 时，[lib.wasm.mode](/config/lib/wasm#wasmmode) 不影响内联行为。当 [lib.wasm](/config/lib/wasm) 设置为 `false` 时，Rslib 不会处理该模块，而是将带有 `?inline` 的导入原样保留在产物中。
- 使用 `new URL('./add.wasm', import.meta.url)` 引用 `.wasm` 文件时，该文件由静态资源流程处理，不受 [lib.wasm](/config/lib/wasm) 影响。
...
使用 TypeScript 时，请先在 `tsconfig.json` 的 `compilerOptions.types` 中添加 `@rslib/core` 提供的 [预设类型](/guide/basic/typescript#预设类型)。如果该配置已有其他类型包，请将它追加到现有数组中：
