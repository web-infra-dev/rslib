---
description: '配置 Rslib，了解 lib 配置、Rsbuild 配置、配置文件与 defineConfig 的使用方式。'
---
...
- [lib 配置](/config/lib)：用于描述库产物，例如模块格式、打包模式和语法目标。
- [Rsbuild 配置](/config/rsbuild)：用于控制编译和构建行为，例如源码转换和产物路径。

Rslib 配置可以配置 `lib` 项中，或者写在顶层。`lib` 项是一个对象数组，每个对象配置一份产物，写在 `lib` 项中的配置用于单独配置该产物。写在顶层的配置则用于所有产物。

### lib 配置

lib 配置通常写在`lib` 项中，用于描述单个产物。其中部分配置也可以写在顶层，作为所有产物的共享配置。`lib` 项内的同名配置优先于共享配置。

例如，所有产物默认使用 [`syntax`](/config/lib/syntax) `es2021`，CJS 产物则单独使用 `es2020`：
...
并非所有 lib 配置都支持共享。例如，`format` 和 `dts` 用于描述单个产物，只能写在 `lib` 项中。完整列表见 [lib 配置](/config/lib)。

### Rsbuild 配置

Rsbuild 配置可以写在顶层，作用于所有产物；也可以写在 `lib` 项中，单独配置该产物。`lib` 项内的 Rsbuild 配置会与顶层配置合并，并在冲突时优先。

例如，所有产物的 [output.target](/config/rsbuild/output#outputtarget) 默认为 `node`，ESM 产物则单独设置为 `web`：
...
你也可以在 [配置总览](/config/) 页面查看所有配置的详细介绍。
