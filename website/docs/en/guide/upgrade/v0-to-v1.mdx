...
Rslib v1 enables Rspack's [`module.parser.javascript.createRequire`](https://rspack.rs/config/module-parser#javascriptcreaterequire). Statically analyzable dependencies loaded through the `require` function returned by `createRequire()` are included in the bundle unless externalized.
...
- If the module should be bundled, reference the source file directly:
...
- If a specific dependency should not be bundled, declare it as external with [output.externals](/config/rsbuild/output#outputexternals).
...
For more details, see [Web Workers - Use the worker constructor](/guide/advanced/web-workers#use-the-worker-constructor).
...
In [bundleless mode](/config/lib/bundle), Rslib v0.x generated the JavaScript code required to load and instantiate Wasm modules. Rslib v1 uses `preserve` mode by default and no longer generates this code. Wasm handling in [bundle mode](/config/lib/bundle) remains unchanged.

If you do not want to preserve `.wasm` imports in a bundleless build, set [`wasm.mode`](/config/lib/wasm#wasmmode) to `'compile'`:
