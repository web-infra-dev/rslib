...
`lib` is an array of objects, with each object representing one output. Both types of configuration can be specified in a `lib` item to customize that output:
...
export default {
  syntax: 'es2021',
  lib: [{ format: 'esm' }, { format: 'cjs', syntax: 'es2020' }],
};
```
