...
Use the `configFileNames` option to replace the default `rslib.config.*` lookup list. The file names are resolved relative to `cwd`, and this option is ignored when `path` is specified.
...
By default, `loadConfig` reads the default export from the config file. Use the `exportName` option to read a named export instead:

```ts
import { loadConfig } from '@rslib/core';

const { content } = await loadConfig({
  exportName: 'rslibConfig',
});
```

Set `exportName` to `false` to execute the config file without reading any exports.
