---
description: '在 Rslib 中选择类型生成方式，使用 TypeScript Compiler API、tsgo、isolatedDeclarations 与 API Extractor 生成和打包类型声明文件。'
---
...
1. 类型生成，即生成 bundleless 类型。支持以下三种方式：
...
2. 类型打包，即生成 bundle 类型（可选）。开启 [dts.bundle](/config/lib/dts#dtsbundle) 时会将生成的类型声明文件通过 [API Extractor](https://api-extractor.com/) 进行打包。
...
| 方式                            | 配置                                          | 类型检查 | 输出范围                | 速度 |
| ------------------------------- | --------------------------------------------- | -------- | ----------------------- | ---- |
| TypeScript Compiler API（默认） | `dts: true` 或 `dts: { bundle: false }`       | 有       | 由 `tsconfig.json` 决定 | 较慢 |
| tsgo                            | [`dts.tsgo`](/config/lib/dts#dtstsgo)         | 有       | 由 `tsconfig.json` 决定 | 快   |
| isolatedDeclarations            | [`dts.isolated`](/config/lib/dts#dtsisolated) | 无       | 构建依赖图中的模块      | 最快 |

#### TypeScript compiler API

默认行为，与执行 `tsc` 的效果基本相同，会生成类型声明文件并执行类型检查，但是速度相对较慢。
...
使用 [native TypeScript](https://github.com/microsoft/typescript-go) 生成类型声明文件，可以在保留类型检查的同时显著提升生成速度。

如果没有显式设置 [dts.tsgo](/config/lib/dts#dtstsgo)，Rslib 会在检测到 TypeScript 7+ 时自动开启该选项。
...
开启 [dts.isolated](/config/lib/dts#dtsisolated) 会使用 Rspack 内置的 SWC fast_dts 能力生成类型声明文件，这种方式速度最快，但不会执行类型检查，并且仅会输出构建依赖图中包含模块的类型声明文件。
...
启用该选项时，建议同时在 `tsconfig.json` 中开启 [isolatedDeclarations](https://www.typescriptlang.org/tsconfig/#isolatedDeclarations)：
...
| [dts.isolated](/config/lib/dts#dtsisolated)                         | 是否使用 `isolatedDeclarations` 生成类型声明文件。                             |
