...
Rslib 的 `loadConfig` 与 Rsbuild 的用法基本一致，区别在于默认查找 `rslib.config.*`，且返回结果中的 `content` 类型为 [RslibConfig](/api/javascript-api/types#rslibconfig)，更多用法可参考 [loadConfig -- Rsbuild](https://rsbuild.rs/zh/api/javascript-api/core#loadconfig)。
