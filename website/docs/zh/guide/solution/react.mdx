...
1. 升级 `react` 和 `react-dom` 到 v19。如果暂时无法升级，可以安装 `react-compiler-runtime`，让编译后的代码在更早版本中运行。
2. React Compiler 目前仅提供 Babel 插件，需要安装 `@rsbuild/plugin-babel` 和 `babel-plugin-react-compiler`。
