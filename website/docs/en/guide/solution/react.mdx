...
In Rslib, you can enable React Compiler through `@rsbuild/plugin-babel` and `babel-plugin-react-compiler`.

### How to use

React Compiler supports React 17, 18, and 19. The following shows the basic setup for React 19:

1. Upgrade `react` and `react-dom` to v19
2. Install `@rsbuild/plugin-babel` and `babel-plugin-react-compiler` as devDependencies
...
::: tip Using React 17 or 18

If you're using React 17 or 18, you need to:

1. Install `react-compiler-runtime` as a production dependency.
2. Configure the `target` option in the Babel plugin configuration:
   ```ts
   babelLoaderOptions(opts) {
     opts.plugins ??= [];
     opts.plugins.unshift([
       ...
       {
         target: '18', // Set to '17' if using React 17
       },
     ]);
   }
   ```
   The `target` option tells the compiler which React version to generate code for.

:::

### Optional: Targeting specific file types

If you only want to compile JSX/TSX files, you can use `include: /\.(?:jsx|tsx)$/` instead of `/\.[jt]sx?$/`, but custom hooks in `.ts` files will not be optimized.
...
See the [react-compiler example](https://github.com/rstackjs/rstack-examples/tree/main/rslib/react-compiler) for a complete configuration.
