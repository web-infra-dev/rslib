...
In addition, Rslib sets the default Rspack `externalsType` based on the output format:

- `esm`: [modern-module](https://rspack.rs/config/externals#externalstypemodern-module). Static `import` externals are emitted as ESM imports, dynamic `import()` externals stay dynamic, and CommonJS `require()` externals are loaded through `createRequire`.
