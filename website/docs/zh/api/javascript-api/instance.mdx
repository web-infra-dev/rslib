...
`rslib.context` 是一个只读对象，提供一些上下文信息，可以通过 Rslib 实例的 `context` 属性访问：
...
当前执行构建的根路径，对应调用 [createRslib](/api/javascript-api/core#createrslib) 时传入的 `cwd` 选项。
...
`context.action` 在运行 CLI 命令或调用 Rslib 实例方法时设置：

- `build`: 当运行 [rslib build](/guide/basic/cli#rslib-build) 或 [rslib.build()](/api/javascript-api/instance#rslibbuild) 时设置。
- `mf-dev`: 当运行 [rslib mf-dev](/guide/basic/cli#rslib-mf-dev) 或 [rslib.startMFDevServer()](/api/javascript-api/instance#rslibstartmfdevserver) 时设置。
- `inspect`: 当运行 [rslib inspect](/guide/basic/cli#rslib-inspect) 或 [rslib.inspectConfig()](/api/javascript-api/instance#rslibinspectconfig) 时设置。
