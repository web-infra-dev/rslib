---
description: '使用 Rslib 核心 API，包括 createRslib、loadConfig、defineConfig 与环境变量加载等 JavaScript 工作流能力。'
---
...
`loadConfig` 的大部分选项用法与 Rsbuild 基本一致。在 Rslib 中，需要注意默认查找的是 `rslib.config.*`，返回的 `content` 类型为 [RslibConfig](/api/javascript-api/types#rslibconfig)。

可参考 Rsbuild 文档中的相同用法：

- [自定义配置文件名](https://rsbuild.rs/api/javascript-api/core#customize-config-file-names)：使用 `configFileNames` 替换默认配置文件查找列表。
- [指定配置文件](https://rsbuild.rs/api/javascript-api/core#specify-the-configuration-file)：使用 `path` 加载相对路径或绝对路径的配置文件。
- [指定导出名称](https://rsbuild.rs/api/javascript-api/core#specify-the-export-name)：使用 `exportName` 读取命名导出，或设置为 `false` 仅执行配置文件而不读取导出。
- [传入 meta 对象](https://rsbuild.rs/api/javascript-api/core#passing-meta-object)：向配置函数传入自定义 `meta` 数据。
- [传入 command](https://rsbuild.rs/api/javascript-api/core#passing-command)：向配置函数传入自定义 `command` 值。
...
对于 [lib](/config/lib) 对象数组中的每一项，会根据 [id](/config/lib/id) 字段处理合并：
