...
配置 CommonJS 和 ESM 输出的 [shims（垫片）](https://developer.mozilla.org/zh-CN/docs/Glossary/Shim)。
...
- `true`：当 [format](/config/lib/format) 为 `esm` 时，在输出的开头将会创建一个由 `createRequire` 生成的 `require`，可以在源代码中像 CommonJS 的全局 `require` 一样使用。
