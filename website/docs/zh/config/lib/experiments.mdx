...
- 禁用代码拆分
- 忽略 [`output.autoExternal`](/config/rsbuild/output#outputautoexternal)、[`output.externals`](/config/rsbuild/output#outputexternals)、[`externalHelpers`](/config/lib/external-helpers) 等配置，将所有模块直接打包进可执行文件
