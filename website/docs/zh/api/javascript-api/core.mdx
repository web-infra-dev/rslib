...
使用 `configFileNames` 选项可以替换默认的 `rslib.config.*` 配置文件查找列表。文件名会相对于 `cwd` 解析，且指定 `path` 时会忽略该选项。
...
默认情况下，`loadConfig` 会读取配置文件的默认导出。你可以通过 `exportName` 选项读取命名导出：
...
将 `exportName` 设置为 `false` 时，只会执行配置文件，不会读取任何导出。
